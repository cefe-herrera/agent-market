...
export default function CreateJob8183({
  ...
  const demoProvider = process.env.NEXT_PUBLIC_ERC8183_DEMO_PROVIDER ?? "";
  const providerInput = selectedPayTo || demoProvider;

  const provider = useMemo(() => {
    if (!isAddress(providerInput)) return null;
    return getAddress(providerInput);
  }, [providerInput]);
  ...
  const ready =
    ...
    Boolean(provider) &&
    Boolean(selectedAgentId || demoProvider) &&
    onPaymentChain;
  ...
  useEffect(() => {
    ...
    setStored(
      loadStoredJobs(cfg.chainId, account).map((item) => item.jobId),
    );
  }, [account, cfg.chainId]);
  ...
  async function onCreate() {
    if (!account || !walletClient || !provider) return;
    setError(null);
    setTx(null);
    setJob(null);
    try {
      ...
      setStep("checking");
      const result = await createAndFundJob(walletClient, {
        provider,
        amount,
        account,
        chainId: cfg.chainId,
  ...
  return (
    <div className="mt-8 border-t border-surface-300 pt-6">
      ...
      <p className="mt-2 font-mono-data text-sm leading-6 text-surface-500">
        Otro riel: fondea un trabajo con escrow $U. No es el hit HTTP de x402.
        El <code>provider</code> es la smart account del agente, no el token
        8004.
      </p>

      <dl className="mt-4 grid gap-2 font-mono-data text-xs">
        <Row label="commerce" value={cfg.commerce} />
        <Row label="router" value={cfg.router} />
        <Row
          label="provider"
          value={
            provider
              ? `${provider}${selectedName ? ` · ${selectedName}` : ""}`
              : "elegí un agente con agentWallet"
          }
          warn={!provider}
        />
        <Row
      ...
      >
        {busy ? `${STEP_LABEL[step] ?? step}…` : "Crear y fondear job 8183"}
      </button>

      {!ready && (
        <p className="mt-2 text-xs text-amber-400">
          Elegí un agente, conectá la wallet en chain {cfg.chainId} y usá un
          `provider` distinto al hit x402. El submit lo firma el Safe del
          agente, no esta wallet.
        </p>
      )}
      ...
      {(step !== "idle" || activeJobId) && (
        <ol className="mt-4 space-y-1 text-[11px]">
          {STEPS.map((name, index) => {
            const done = step === "done" || (step !== "idle" && index < stepIndex);
            ...
            <button
              ...
              onClick={() =>
                void onJobAction("refund", () =>
                  claimRefund(walletClient!, job.id, {
                    account: account!,
                    chainId: cfg.chainId,
                  }),
                )
