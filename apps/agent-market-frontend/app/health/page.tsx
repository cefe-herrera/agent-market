...
import HealthAgentCard from "@/app/components/marketplace/HealthAgentCard";
import { healthMarketplaceAgents } from "@/app/lib/gemini/catalog";
...
export default function HealthPage() {
  const { t } = useI18n();
  const agents = healthMarketplaceAgents();
  const [selected, setSelected] = useState<MarketplaceAgent>(agents[0]);

  return (
    <div className="flex-1">
      <section className="page-section">
        <p className="label-terminal mb-3">// {t("health.badge")}</p>
        <h1 className="font-pixel-square text-2xl text-surface-950">
          {t("health.title")}
        </h1>
        <p className="page-hero-subtitle">{t("health.subtitle")}</p>
      </section>
