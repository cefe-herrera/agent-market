...
Angular (`apps/web`) **salió del producto**. El UI es Next. Nest es el único que habla con facilitator e indexer.
...
## Cómo correr (hoy, a mano)

Hace falta un **Makefile o `scripts/dev.sh`** que levante Nest + Next + (opcional) indexer/facilitator locales. Todavía no está. Mientras tanto:

```bash
# Nest :3000  — FACILITATOR_URL + INDEXER_BNB_URL en el .env raíz
npm run dev:api:mainnet

# Next UI :3001  — NEST_* en apps/agent-market-frontend/.env.local
